...
export class WasmTopologyResult {
     ...
     */
    get case_ids() {
        const ret = wasm.wasmtopologyresult_case_ids(this.__wbg_ptr);
     ...
     */
    get max_stress() {
        const ret = wasm.wasmtopologyresult_max_stress(this.__wbg_ptr);
        return ret;
    }
    /**
     * @returns {number}
 ...
 */
export function optimize_assembly_frame(preset, input) {
    const ptr0 = passStringToWasm0(preset, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.optimize_assembly_frame(ptr0, len0, input);
    if (ret[2]) {
 ...
 * @returns {number}
 */
export function relative_l2(expected, actual) {
    const ptr0 = passArrayF32ToWasm0(expected, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArrayF32ToWasm0(actual, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.relative_l2(ptr0, len0, ptr1, len1);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return ret[0];
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('webmcp_reference_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
