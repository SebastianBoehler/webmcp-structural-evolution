...
const mechanismInputDigest = z.string().regex(/^[0-9a-f]{64}$/);
...
export const MechanismSolverRequestSchema = z.discriminatedUnion("type", [
  z.object({ type: z.literal("solve-mechanism"), requestId, mechanismInputDigest,
    inputBytes: ownedBytes(32 * 1024 * 1024) }).strict(),
  z.object({ type: z.literal("cancel"), requestId }).strict(),
]);

export const MechanismSolverEventSchema = z.discriminatedUnion("type", [
  z.object({ type: z.literal("started"), requestId, mechanismInputDigest }).strict(),
