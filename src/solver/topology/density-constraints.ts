export function topologyMask(density: Float32Array, isoValue: number): Uint32Array {
  return Uint32Array.from(density, (value) => Number(value >= isoValue));
}
...
export function projectTopologyDensity(
  ...
  if (candidate.length !== previous.length || candidate.some((value) => !Number.isFinite(value))) {
    throw new Error("Topology density update is invalid");
  }
  const output = new Float32Array(candidate.length);
  for (let index = 0; index < output.length; index += 1) {
    if (required.has(index)) output[index] = 1;
  ...
  const targetTotal = targetVolumeFraction * output.length;
  if (required.size > targetTotal + 1e-6) {
    throw new Error("Topology target volume cannot preserve every required interface cell");
  }
  const design = [...output.keys()].filter((index) => !required.has(index) && !protectedCells.has(index));
