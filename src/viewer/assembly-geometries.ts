...
function flightControllerPieces(part: Extract<AssemblyVisualPart, { kind: "flight-controller" }>): readonly VisualPiece[] {
  ...
  const pieces: VisualPiece[] = [
    { id: "flight-controller-board", geometry: new THREE.BoxGeometry(width, height, depth * 0.72), color: 0x252d36, metalness: 0.18 },
    { id: "flight-controller-pcb", geometry: new THREE.BoxGeometry(width - 3, height - 3, 1.2), color: 0x244b42, position: [0, 0, depth * 0.38] },
    { id: "flight-controller-chip", geometry: new THREE.BoxGeometry(9, 9, 1.8), color: 0x151a20, metalness: 0.32, position: [0, 0, depth * 0.49] },
    { id: "flight-controller-connector-left", geometry: new THREE.BoxGeometry(7, 6, 2), color: 0x171c22, position: [-12, 6, depth * 0.49] },
    { id: "flight-controller-connector-right", geometry: new THREE.BoxGeometry(7, 6, 2), color: 0x171c22, position: [12, -6, depth * 0.49] },
  ];
  for (const side of [-1, 1]) {
    for (let index = -1; index <= 1; index += 1) {
      pieces.push({
        id: `flight-controller-header-${side < 0 ? "left" : "right"}-${index + 2}`,
...
export function geometryPieces(part: Exclude<AssemblyVisualPart, { kind: "model" | "mesh" }>): readonly VisualPiece[] {
  if (part.kind === "box") return [{ id: "box", geometry: new THREE.BoxGeometry(...part.size) }];
  if (part.kind === "cylinder") return [{ id: "cylinder", geometry: cylinder(part.radius, part.height) }];
  ...
  if (part.kind === "load-vector") return [
    { id: "load-vector-shaft", geometry: cylinder(1.2, part.length * 0.68).rotateX(Math.PI / 2), color: 0xe04d3f, position: [0, 0, -part.length * 0.34] },
    { id: "load-vector-head", geometry: new THREE.ConeGeometry(3.2, part.length * 0.32, 32).rotateX(Math.PI / 2), color: 0xe04d3f, position: [0, 0, -part.length * 0.84] },
  ];
