...
export type ReferenceGeometry =
  | Readonly<{ kind: "parametric"; graph: ParametricGraph; display: MotorDisplay | FastenerDisplay }>
...
type MotorDisplay = Readonly<{
  ...
  shaftRadius: number;
  shaftHeight: number;
  mountPitchCircle: number;
}>;
type FastenerDisplay = Readonly<{ kind: "fastener"; shankRadius: number; shankLength: number; headRadius: number; headHeight: number }>;
...
const source = (title: string, url: string): Source => ({
  title, url, accessedOn: "2026-08-26", redistribution: "specification-facts-only; no CAD redistributed",
});
...
const fastenerGraph: ParametricGraph = { nodes: [
  ...
  cylinder("socket-head", [0, 0, 0.0095], 0.00284, 0.003),
  { kind: "union", id: "m3x8-solid", left: "m3-thread-envelope", right: "socket-head" },
  { kind: "box", id: "socket-recess", center: point(0, 0, 0.0107), size: point(0.0025, 0.0025, 0.0014) },
...
export const REFERENCE_DRONE_CATALOG: readonly ReferenceDroneComponent[] = Object.freeze([
  { id: "motor-2207", exactPart: "Hobbywing XRotor-2207.5SL-1780KV", category: "motor", massKg: 0.038, optimizationRole: "fixed-component",
    geometry: { kind: "parametric", graph: motorGraph, display: { kind: "motor", radius: 0.014, bodyHeight: 0.0199, statorRadius: 0.01125, statorHeight: 0.0076, shaftRadius: 0.0025, shaftHeight: 0.012, mountPitchCircle: 0.016 } },
    ...
    provenance: { mode: "modeled-from-specification", sources: [hobbywing, hobbywingProduct], dimensionalUncertainty: "Published dimensions; unlisted bell/base contours modeled within the 28 mm x 19.9 mm envelope." } },
  { id: "fc-esc-stack-30x30", exactPart: "SpeedyBee F405 V4 + BLS 55A Stack, SB-F4V4-55-STACK", category: "avionics", massKg: 0.034, optimizationRole: "fixed-component",
    ...
    provenance: { mode: "modeled-from-specification", sources: [speedybee], dimensionalUncertainty: "Board dimensions and masses are published; 4 mm installed board gap is a conservative +/-2 mm assembly assumption." } },
  { id: "battery-6s-1550", exactPart: "Tattu TA-RL5-150C-1550-6S1P", category: "battery", massKg: 0.254, optimizationRole: "fixed-component", geometry: { kind: "box", size: [0.078, 0.037, 0.052] }, collision: [{ kind: "box", center: [0, 0, 0], size: [0.078, 0.037, 0.052] }], interfaces: [{ id: "battery-power", kind: "cable", position: [0.039, 0, 0], connector: "XT60" }], protectedEnvelopes: [{ kind: "box", center: [0, 0, 0], size: [0.084, 0.043, 0.058] }], provenance: { mode: "modeled-from-specification", sources: [tattu], dimensionalUncertainty: "Manufacturer tolerances: length +/-5 mm, width/height +/-2 mm, mass +/-20 g." } },
  { id: "fastener-m3x8", exactPart: "Accu SSCF-M3-8-12.9-Z", category: "fastener", massKg: 0.0008, optimizationRole: "fixed-component", geometry: { kind: "parametric", graph: fastenerGraph, display: { kind: "fastener", shankRadius: 0.0015, shankLength: 0.008, headRadius: 0.00284, headHeight: 0.003 } }, collision: [{ kind: "cylinder", center: [0, 0, 0.0055], radius: 0.00284, height: 0.011 }], interfaces: [{ id: "m3-thread", kind: "mate", position: [0, 0, 0], diameter: 0.003 }], protectedEnvelopes: [], provenance: { mode: "modeled-from-specification", sources: [accu], dimensionalUncertainty: "Head diameter +0/-0.36 mm and head height +0/-0.14 mm; thread represented by its nominal envelope." } },
  { id: "motor-wiring-corridor", exactPart: "Reference 3-phase routing corridor", category: "wiring", massKg: 0, optimizationRole: "protected", geometry: { kind: "corridor", size: [0.184, 0.006, 0.006] }, collision: [{ kind: "box", center: [0, 0, 0], size: [0.184, 0.006, 0.006] }], interfaces: [{ id: "wire-route", kind: "cable", position: [0, 0, 0], connector: "3x 20AWG motor leads" }], protectedEnvelopes: [{ kind: "box", center: [0, 0, 0], size: [0.184, 0.006, 0.006] }], provenance: { mode: "modeled-from-specification", sources: [hobbywing, speedybee], dimensionalUncertainty: "6 mm corridor is a conservative routing allowance around published 20AWG motor leads; route is assembly-defined." } },
  { id: "propeller-5x4.3x3", exactPart: "HQProp HQ5X4.3X3V2S-PC", category: "propeller", massKg: 0.0038, optimizationRole: "protected", geometry: { kind: "swept-rotor", radius: 0.0635, hubRadius: 0.0064, hubHeight: 0.0065, bladeCount: 3 }, collision: [{ kind: "swept-disc", center: [0, 0, 0], radius: 0.0635, height: 0.0065 }], interfaces: [{ id: "propeller-shaft", kind: "mate", position: [0, 0, 0], diameter: 0.005 }], protectedEnvelopes: [{ kind: "swept-disc", center: [0, 0, 0], radius: 0.066, height: 0.0085 }], provenance: { mode: "modeled-from-specification", sources: [hqprop], dimensionalUncertainty: "Published overall and hub dimensions; blade planform is illustrative and the protected radius includes 2.5 mm clearance." } },
]);
...
const motors: readonly ReferenceDroneInstance[] = motorCenters.map(([id, x, y]) => ({ id, componentId: "motor-2207", position: [x, y, 0.012], yaw: 0, optimizationRole: "fixed-component" }));
const props: readonly ReferenceDroneInstance[] = motorCenters.map(([id, x, y]) => ({ id: `${id}-propeller`, componentId: "propeller-5x4.3x3", position: [x, y, 0.03], yaw: 0, optimizationRole: "protected" }));
const fasteners: readonly ReferenceDroneInstance[] = motorCenters.flatMap(([id, x, y]) => mountPoints.map(([dx, dy], index) => ({ id: `${id}-fastener-${index + 1}`, componentId: "fastener-m3x8", position: [x + dx, y + dy, 0.0015], yaw: 0, optimizationRole: "fixed-component" })));
export const referenceDroneAssembly = Object.freeze({ id: "reference-5-inch-drone", units: "m" as const, instances: Object.freeze<readonly ReferenceDroneInstance[]>([
