...
| 0:38–1:25 | Paste Prompt 2. Keep the shared 3D world visible, then show the running status and the resulting candidate in **Optimize**. | “From that exact revision, I ask for one balanced topology candidate. The browser runs a bounded deterministic sparse-SIMP lattice estimate in a same-origin worker with Rust and Wasm. The assembly revision, named loads, supports, and protected volumes travel together. In our production rehearsal this completed in about thirteen seconds and returned a reviewable Balanced frame estimate. It is useful immediately, but it is still an estimate, not a manufacturing decision.” |
| 1:25–1:58 | Open **Review → Evidence**, then **History**. Show prediction, measured output, plan state, and the receipt. | “The review surface keeps four things separate: what the agent predicted, what the interactive run measured, whether the plan is still current, and who has authority. The receipt identifies the action and revision. If I change a relevant part or constraint, the prior plan becomes stale instead of pretending to stay current. The screenshots, numbers, and controls stay tied to the same branch I am reviewing. That keeps the conversation attached to the exact state in front of me.” |
...
- **Optimize** showing the balanced candidate and its result panel, followed by **Review → Evidence**, **History**, and the disabled **Use this frame** control.
