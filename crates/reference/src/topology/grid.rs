...
pub(crate) fn drone_grid() -> Grid {
    ...
    let motor_centers = [[100.0, 0.0], [-100.0, 0.0], [0.0, 100.0], [0.0, -100.0]];
    ...
    for z in 0..depth {
        ...
        for y in 0..height {
            ...
            for x in 0..width {
                ...
                let mount = wz.abs() <= 5.0
                    && motor_centers.iter().any(|center| disk(wx, wy, center[0], center[1], 15.0));
                let core_ring = wx.abs() <= 30.0 && wy.abs() <= 30.0
                    && (wx.abs() >= 20.0 || wy.abs() >= 20.0) && wz.abs() <= 5.0;
                let avionics = wx.abs() < 18.0 && wy.abs() < 18.0 && wz >= -5.0;
                ...
                passive_void.push(!solid_interface && (!inside_domain || avionics || cable_channel));
            }
        }
    }
